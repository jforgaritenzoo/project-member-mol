...
load_dotenv()
oracledb.init_oracle_client(lib_dir="D:\Code\instantclient-basiclite-windows.x64-23.6.0.24.10\instantclient_23_6")
...
def get_source_conn():
    try:
        source_conn = create_engine(
            url="oracle+oracledb://{0}:{1}@{2}:1521/orcl".format(user3, password3, host3)
        ).connect()
...
def get_crm_conn():
    try:
        crm_conn = create_engine(
            url="oracle+oracledb://{0}:{1}@{2}:1521/igrcrm".format(usercrm, passwordcrm, hostcrm)
        ).connect()
...
def get_local_conn():
    try:
        localdb_conn = create_engine(
            url="mysql+pymysql://{0}:{1}@{2}/{3}".format(user2, password2, host2, database2)
        ).connect()
...
def get_app_conn():
    try:
        app_conn = create_engine(
            url="mysql+pymysql://{0}:{1}@{2}/{3}".format(user2, password2, host2, database4)
        ).connect()
